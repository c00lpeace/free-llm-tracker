...
- 각 제공자의 무료 모델은 **사용성 좋은 순으로 대표 3개만 표시**하고, 나머지는 '더보기'를 눌러 펼쳐볼 수 있게 접어둠 (GitHub 웹에서 펼쳐보기 가능).
- 정기 체크(매일 06:00 / 18:00 KST)는 본문을 항상 최신 상태로 갱신하고,
  변경된 사실은 맨 아래 '변경 이력'에 날짜순으로 한 줄씩 추가함.

## Groq (이전 조사) — 우선순위: 높음

- 대표 무료 모델 (사용성 순): `gpt-oss-120b` (한도 문서에 명시된 무료 모델), Llama 3.3 70B 계열, Qwen3 계열
<details>
<summary>더보기 — 전체 무료 모델 안내</summary>

- GPT/Claude/Gemini 계열은 없음. 정확한 전체 목록은 가입 후 계정 limits 페이지에서 확인 필요.
</details>
...
## NVIDIA NIM (build.nvidia.com) — 우선순위: 높음

- 대표 무료 모델 (사용성 순): `moonshotai/kimi-k2.5` (function calling 광고), `deepseek-ai/deepseek-v4-flash-0731` (고속 플래그십), `meta/llama-3.3-70b-instruct` (안정적 폴백)
<details>
<summary>더보기 — 전체 무료 모델 목록</summary>

- `nvidia/nemotron-nano-3-30b-a3b`, `meta/muse-glimmer-30b`, `deepseek-ai/deepseek-v3.2` 등 카탈로그 100+ 오픈웨이트 모델 (ID는 카탈로그에서 복사 필요 — 예고 없이 변경됨)
</details>
...
- 대표 무료 모델 (사용성 순): `deepseek/deepseek-v4-pro-free` (최상위 성능), `deepseek/deepseek-v4-flash-free` (고속), `tencent/hy3-free`
<details>
<summary>더보기 — 전체 무료 모델 안내</summary>

- `orcarouter/free` 자동 라우팅 별칭도 제공. 무료 라인업은 로테이션됨 (2026-09-07 이후 `qwen/qwen3.8-27b-free` → GLM-5.3 Flash 교체 보고).
</details>
...
## Google AI Studio — Gemini (이전 조사) — 우선순위: 중간

- 대표 무료 모델 (사용성 순): Gemini 2.5 Flash-Lite (일 1,000~1,500회 — 한도 최다), Gemini 2.5 Flash, Gemma 계열
<details>
<summary>더보기 — 무료 모델 범위 안내</summary>

- Flash 계열만 무료 (Gemini 2.5/3.x Flash, Flash-Lite, Gemma). Pro 모델은 2026년 4월부터 무료 티어 제외 (유료 전용). 한도는 프로젝트·리전별 변동이 큼.
</details>
...
## OpenCode Zen — 우선순위: 중간

- 대표 무료 모델 (사용성 순): `nemotron-3-ultra-free` (플래그십), `mimo-v2.6-flash-free` (고속), `big-pickle`
<details>
<summary>더보기 — 전체 무료 모델 목록</summary>

- `space-bunny-free`, `mimo-v2.5-free`, `ling-3.0-flash-fin-free`, `nemotron-3.5-lightning-free`, `muse-spark-1.3-contributor-free`, `jev-1.13-free` (공식 문서 가격표 기준 "Free" 행 — 로테이션됨)
</details>
...
## Mistral (이전 조사) — 우선순위: 중간

- 대표 무료 모델 (사용성 순): Mistral Large 계열 (최상위 성능), Codestral (코드 특화), Mistral Small 계열 (경량·고속)
<details>
<summary>더보기 — 전체 무료 모델 안내</summary>
...
## OpenRouter (이전 조사) — 우선순위: 중간

- 대표 무료 모델 (사용성 순, 2026-09 기준 예시 — 라인업 로테이션됨): `deepseek/deepseek-r1:free`, `qwen/qwen3-235b-a22b:free`, `meta-llama/llama-3.3-70b-instruct:free`
<details>
<summary>더보기 — 전체 무료 모델 안내</summary>

- 모델명 끝에 `:free`가 붙는 모델들 (2026-09-11 기준 약 19개 텍스트 모델 — 로테이션됨)
</details>
...
## Cerebras (이전 조사) — 우선순위: 낮음

- 대표 무료 모델: `gpt-oss-120b`, `qwen-3.8-27b` (무료 티어 8K 컨텍스트 제한 보고 있음)
...
## GitHub Models — 우선순위: 낮음

- 대표 무료 모델 (사용성 순): `openai/gpt-5`, `openai/o3`, `deepseek/DeepSeek-R1`
<details>
<summary>더보기 — 전체 무료 모델 목록</summary>
...
- 대표 무료 모델 (사용성 순): `GLM-5.3-Flash`, `minimax-m2.7`, `codestral-latest`
<details>
<summary>더보기 — 전체 무료 모델 목록</summary>
